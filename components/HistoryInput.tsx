...
const HistoryInput: React.FC<HistoryInputProps> = ({ onAnalyze, isLoading }) => {
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-2xl shadow-lg border border-slate-700">
      <h2 className="text-xl font-semibold mb-4 text-slate-200">Geçmiş Verilerinizi Girin</h2>
      ...
      {mode === 'text' && (
        <div>
          <textarea
            ...
            disabled={isLoading}
            placeholder="Tarayıcı geçmişinizi buraya yapıştırın..."
            className="w-full h-48 p-4 bg-slate-900 border border-slate-600 rounded-lg text-slate-300 placeholder-slate-500 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-colors duration-200 resize-none"
          />
        </div>
      )}
